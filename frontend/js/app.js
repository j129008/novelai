...
function setupPoseExplore() {
  const overlay    = document.getElementById("pose-explore-overlay");
  const closeBtn   = document.getElementById("pose-explore-close");
  const imgArea    = document.getElementById("pose-explore-img-area");
  const hintInput  = document.getElementById("pose-explore-hint");
  const prevBtn    = document.getElementById("pose-explore-prev");
  const nextBtn    = document.getElementById("pose-explore-next");
  const stylizeBtn = document.getElementById("pose-explore-stylize");
  const openBtn    = document.getElementById("cpb-explore-btn");
  if (!overlay || !openBtn) return;
  ...
  function close() { overlay.style.display = "none"; history = []; historyIdx = -1; resetUI(); }

  function resetUI() {
    imgArea.innerHTML = '<div class="pose-explore-skeleton"></div>';
    prevBtn.style.display  = "none";
    stylizeBtn.style.display = "none";
    nextBtn.textContent = "Generate First";
  ...
  function showImage(b64) {
    ...
    stylizeBtn.style.display = "";
    nextBtn.textContent = "Next \u2192";
    prevBtn.style.display = historyIdx > 0 ? "" : "none";
  }
  ...
  async function generate() {
    ...
    imgArea.innerHTML = '<div class="pose-explore-skeleton"></div>';
    try {
      const resp = await fetch("/api/pose-explore", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ hint: hintInput.value.trim() }),
      });
      if (!resp.ok) throw new Error("Failed");
      const data = await resp.json();
      history.push({ b64: data.image, hint: hintInput.value });
      if (history.length > 10) history.shift();
      historyIdx = history.length - 1;
      showImage(data.image);
    } catch (e) {
      imgArea.innerHTML = '<p style="color:var(--danger);font-size:0.8rem">Failed to generate</p>';
    } finally {
      loading = false;
      nextBtn.disabled = false;
    }
  ...
  prevBtn.addEventListener("click", () => {
    if (historyIdx > 0) { historyIdx--; showImage(history[historyIdx].b64); }
  });
...
function setupAutoCritique() {
  const panel      = document.getElementById("critique-panel");
  const body       = document.getElementById("critique-body");
  const dismissBtn = document.getElementById("critique-dismiss");
  const applyAllBtn = document.getElementById("critique-apply-all");
  const toggle     = document.getElementById("auto-critique-toggle");
  if (!panel || !body || !toggle) return;

  // Restore from localStorage
  toggle.checked = localStorage.getItem("nai-auto-critique") === "true";
  toggle.addEventListener("change", () => {
    localStorage.setItem("nai-auto-critique", toggle.checked);
    if (!toggle.checked) panel.style.display = "none";
  });
  ...
  // Hook into generation complete — observe #info-seed changes
  ...
  if (infoSeed) {
    new MutationObserver(() => {
      ...
      runCritique();
    }).observe(infoSeed, { childList: true, characterData: true, subtree: true });
  }

  async function runCritique() {
    panel.style.display = "flex";
    body.innerHTML = '<div class="critique-skeleton"></div>';
    ...
    try {
      const resp = await fetch("/api/critique", {
        ...
        body: JSON.stringify({
          ...
          current_prompt: document.getElementById("prompt")?.value || "",
        }),
      });
      if (!resp.ok) throw new Error("Failed");
      const data = await resp.json();
      _currentBullets = data.bullets || [];
      renderBullets();
    } catch {
      // Silent failure — critique is optional
  ...
  function renderBullets() {
    body.innerHTML = "";
    if (!_currentBullets.length) { panel.style.display = "none"; return; }

    _currentBullets.forEach(bullet => {
      const div = document.createElement("div");
      div.className = "critique-bullet";

      const text = document.createElement("p");
      text.className = "critique-bullet-text";
      text.textContent = "\u2022 " + bullet.text;
      div.appendChild(text);

      const tags = document.createElement("div");
      tags.className = "critique-tags";

      (bullet.add_tags || []).forEach(tag => {
        const chip = document.createElement("button");
        chip.className = "critique-tag critique-tag--add";
        chip.type = "button";
        chip.textContent = "+ " + tag;
        chip.addEventListener("click", () => {
          if (chip.classList.contains("critique-tag--applied")) {
            removeTagFromPrompt(tag);
            chip.classList.remove("critique-tag--applied");
          } else {
            insertTagIntoPrompt(tag);
            chip.classList.add("critique-tag--applied");
          }
        });
        tags.appendChild(chip);
      });

      (bullet.remove_tags || []).forEach(tag => {
        const chip = document.createElement("button");
        chip.className = "critique-tag critique-tag--remove";
        chip.type = "button";
        chip.textContent = "\u2212 " + tag;
        chip.addEventListener("click", () => {
          if (chip.classList.contains("critique-tag--applied")) {
  ...
  // Helper: insert tag at end of prompt
  function insertTagIntoPrompt(tag) {
    const el = document.getElementById("prompt");
    if (!el) return;
    const val = el.value.trim();
    const display = tag.replace(/_/g, " ");
    el.value = val ? val + ", " + display : display;
    el.dispatchEvent(new Event("input", { bubbles: true }));
  }

  // Helper: remove tag from prompt (handles solo-tag case per learnings)
  function removeTagFromPrompt(tag) {
    const el = document.getElementById("prompt");
    if (!el) return;
    const display = tag.replace(/_/g, " ");
    const currentTags = el.value.split(",").map(t => t.trim());
    const filtered = currentTags.filter(t => t.toLowerCase() !== display.toLowerCase());
  ...
  applyAllBtn.addEventListener("click", () => {
    _currentBullets.forEach(bullet => {
      (bullet.add_tags || []).forEach(t => insertTagIntoPrompt(t));
      (bullet.remove_tags || []).forEach(t => removeTagFromPrompt(t));
    });
    panel.style.display = "none";
